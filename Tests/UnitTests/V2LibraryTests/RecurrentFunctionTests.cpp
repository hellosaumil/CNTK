...
BOOST_AUTO_TEST_CASE(TestParityCandCppLSTMModel)
{
    ...
    RequireClose(result1, result3, 0.1);
    RequireClose(result1, result4, 0.1);
    ...
    // Sequence mode.
    {
        ...
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> cresult1(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
        ...
        rc = CNTK_EvaluateSequence(model, argumentInfos, &iv, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> cresult2(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
        ...
        rc = CNTK_EvaluateSequence(model, argumentInfos, &iv, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> cresult3(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
        ...
        rc = CNTK_EvaluateSequence(model, argumentInfos, &iv, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> cresult4(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
        ...
        RequireClose(result, cresult1, 0.1);
        RequireClose(result1, cresult1, 0.1);
        RequireClose(result2, cresult2, 0.1);
        RequireClose(result3, cresult3, 0.1);
        RequireClose(result4, cresult4, 0.1);
    ...
    // Frame mode.
    {
        ...
        frame1.dataSize = (uint32_t)inputData.size() / numberOfFrames;
        frame1.shape = shape;
        ...
        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame1, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> resultFrame1(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);

        // Second frame without reset.
        auto frame2 = frame1;
        frame2.data += frame1.dataSize;
        sequenceFlags[0] = false;
        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame2, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> resultFrame2(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);

        // Third frame without reset.
        auto frame3 = frame2;
        frame3.data += frame2.dataSize;
        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame3, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        std::vector<float> resultFrame3(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
        ...
        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame1, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        resultFrame1.assign(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);

        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame2, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        resultFrame2.assign(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);

        rc = CNTK_EvaluateSequence(model, argumentInfos, &frame3, sequenceFlags, numArguments,
            outputInfos, numOutputs, &outputValues);
        BOOST_CHECK_EQUAL(rc.value, CNTK_SUCCESS);
        resultFrame3.assign(outputValues[0].data, outputValues[0].data + outputValues[0].dataSize);
